...
internal static class WorkspaceEditPayloadCloner
{
	...
	/// <summary>
	/// Clones the change-annotation map into a detached read-only snapshot.
	/// </summary>
	/// <param name="changeAnnotations">The change annotations to clone.</param>
	/// <returns>The cloned change annotations, or <see langword="null"/> when none were provided.</returns>
	public static IReadOnlyDictionary<string, WorkspaceEditChangeAnnotationPayload>? CloneChangeAnnotations(
		IReadOnlyDictionary<string, WorkspaceEditChangeAnnotationPayload>? changeAnnotations)
		=> changeAnnotations is null
			? null
			: new ReadOnlyDictionary<string, WorkspaceEditChangeAnnotationPayload>(
				new Dictionary<string, WorkspaceEditChangeAnnotationPayload>(changeAnnotations, StringComparer.Ordinal));
}
