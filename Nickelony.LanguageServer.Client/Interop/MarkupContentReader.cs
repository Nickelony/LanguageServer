...
public static class MarkupContentReader
{
	...
	/// A bare string payload is treated as Markdown, matching the LSP <c>MarkedString</c> definition; servers that
	/// use the string form for raw code will therefore be rendered as Markdown.
	/// </remarks>
	public static ProtocolMarkupContent ExtractContent(JsonElement element) => element.ValueKind switch
	{
		JsonValueKind.String => new ProtocolMarkupContent(element.GetString(), true),
		...
		JsonValueKind.Object when TryGetStringProperty(element, "language", out string? language)
			&& TryGetStringProperty(element, "value", out string? codeValue)
				=> new ProtocolMarkupContent(BuildFencedCodeBlock(language, codeValue), true),

		JsonValueKind.Object when TryGetStringProperty(element, "value", out string? plainValue)
			=> new ProtocolMarkupContent(plainValue, false),
