...
/// </summary>
/// <remarks>
/// The job handle and the first-wins diagnostics logger are process-wide state shared by every client instance:
/// all language-server processes of one process are assigned to the same job object, later clients cannot replace
/// the logger, and the handle intentionally lives until the process ends.
/// </remarks>
...
internal static class ProcessJobObject
{
	...
	private static int s_loggerInitialized;
	...
	/// <summary>
	/// Sets the logger used for job-object diagnostics. The first initialized logger wins.
	/// </summary>
	/// <remarks>
	/// The job object is process-wide state, so the first initialized logger is used for all job-object
	/// diagnostics in the process and later clients cannot replace it.
	/// </remarks>
	...
	{
		if (Interlocked.CompareExchange(ref s_loggerInitialized, 1, 0) != 0)
			return;
	...
	internal static void TryAssignProcess(Process process)
	{
	...
	private static IntPtr EnsureJobHandle()
	{
		...
		IntPtr jobHandle = Volatile.Read(ref s_jobHandle);

		if (jobHandle != IntPtr.Zero)
			return jobHandle;

		if (Volatile.Read(ref s_initializationFailed))
			return IntPtr.Zero;
