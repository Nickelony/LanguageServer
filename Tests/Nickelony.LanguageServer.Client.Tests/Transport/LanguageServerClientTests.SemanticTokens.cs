...
public partial class LanguageServerClientTests
{
	...
	public async Task RefreshSemanticTokensAsync_IgnoresStaleTransportGeneration()
	{
		...
		LanguageServerClientRpcTarget oldRpcTarget = CreateRpcTarget(client, GetTransportGeneration(oldSession));
		object? result = await oldRpcTarget.RefreshSemanticTokensAsync().ConfigureAwait(false);

		// A broken gate would enqueue the refresh for asynchronous callback dispatch; give the pump a bounded
		// window to deliver it before asserting the drop.
