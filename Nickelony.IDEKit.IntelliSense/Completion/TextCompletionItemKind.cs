...
/// hosts may use <see cref="CreateCustom(string)"/> for additional categories without changing this assembly.
/// Identifiers are normalized by trimming surrounding whitespace, compared with ordinal case-sensitive equality,
/// and serialized as strings. A renderer that does not recognize a custom identifier should use
/// <see cref="Generic"/> presentation while preserving the original <see cref="Identifier"/> value in the model.
/// </remarks>
...
public sealed class TextCompletionItemKind : IEquatable<TextCompletionItemKind>
{
	...
	private static string NormalizeAndValidate(string identifier)
	{
		...
		if (normalizedIdentifier.Length > MaximumIdentifierLength)
			throw new ArgumentException(
				$"A completion-kind identifier cannot exceed {MaximumIdentifierLength} characters.",
				nameof(identifier));

		if (!IsIdentifierStart(normalizedIdentifier[0]))
			throw new ArgumentException(
				"A completion-kind identifier must start with a letter or underscore.",
				nameof(identifier));
		...
		for (int i = 1; i < normalizedIdentifier.Length; i++)
		{
			if (!IsIdentifierPart(normalizedIdentifier[i]))
				throw new ArgumentException(
					"A completion-kind identifier may contain only letters, digits, underscore, period, hyphen, or colon.",
					nameof(identifier));
		}
